...
int KmBoxNetManager::InitDevice(const std::string& IP, WORD Port, const std::string& Mac)
{
    ...
    LOG("Successfully connected to KMBOX:");
    LOG("  IP: %s", IP.c_str());
    LOG("  Port: %d", Port);
    LOG("  UUID: %s", Mac.c_str());
...
void KmBoxNetManager::SpeedTest(int count)
{
    ...
    for (int i = count; i > 0; i -= 2) { // decrement by 2 as we are making two calls in each iteration
        ...
        if (ret != 0) {
            LOG_ERROR("tx error %d ret1=%d", i, ret);
        }

        ret = Kmbox.Mouse.Move(0, 100);
        if (ret != 0) {
            LOG_ERROR("tx error %d ret2=%d", i, ret);
        }
    }

    auto endTime = std::chrono::steady_clock::now();
    auto elapsedTime = std::chrono::duration_cast<std::chrono::milliseconds>(endTime - startTime).count();
    LOG("Speed test (%d calls) took %lld ms", count, elapsedTime);
}
